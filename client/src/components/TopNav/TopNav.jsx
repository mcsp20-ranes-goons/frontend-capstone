import { useState } from 'react';
import { TfiWorld } from 'react-icons/tfi';
import { AiOutlineUserAdd } from 'react-icons/ai';

function TopNav() {
	const [selectedLanguage, setSelectedLanguage] = useState('en');
	const [selectedLink, setSelectedLink] = useState('store');

	const languages = [
		{ code: 'en', label: 'English' },
		{ code: 'es', label: 'Español' },
		{ code: 'fr', label: 'Français' },
		{ code: 'de', label: 'Deutsch' },
		{ code: 'hi', label: 'Hindi' },
	];
	const [showMenu, setShowMenu] = useState(false);
	const handleLanguageChange = (code) => setSelectedLanguage(code);
	const handleMenuClick = () => setShowMenu(!showMenu);
	return (
		<header
			id='header'
			className='w-screen bg-[#2a2a2a]  flex justify-between items-center'
		>
			{/* Logo */}
			<div className='w-0.5/12 flex items-center justify-start px-3'>
				<img
					src='data:image/png;base64,'
					alt='Logo'
					className='w-12 h-12 m-3'
				/>
			</div>

			{/* Navigation */}
			<nav className='lg:w-10/12 md:w-9/12 hidden md:flex items-center'>
				<a
					onClick={(e) => {
						e.preventDefault();
						setSelectedLink('store');
					}}
					href='/'
					className={`mx-3 text-[#ccc] p-5  ${
						selectedLink === 'store' ? 'border-b-4 border-[#007aff]' : ''
					}`}
				>
					STORE
				</a>
				<a
					onClick={(e) => {
						e.preventDefault();
						setSelectedLink('distribution');
					}}
					href='/'
					className={`mx-3 text-[#ccc] p-5  ${
						selectedLink === 'distribution' ? 'border-b-4 border-[#007aff]' : ''
					}`}
				>
					DISTRIBUTION
				</a>
				<a
					onClick={(e) => {
						e.preventDefault();
						setSelectedLink('support');
					}}
					href='/'
					className={`mx-3 text-[#ccc] p-5  ${
						selectedLink === 'support' ? 'border-b-4 border-[#007aff]' : ''
					}`}
				>
					SUPPORT
				</a>
				<a
					onClick={(e) => {
						e.preventDefault();
						setSelectedLink('engine');
					}}
					href='/'
					className={`border-l-2 px-10 border-[#ccc] mx-3 text-[#ccc] p-5  ${
						selectedLink === 'engine' ? 'border-b-4 border-[#007aff]' : ''
					}`}
				>
					UNREAL ENGINE
				</a>
			</nav>
			{/* Authorisation buttons */}
			<div className='md:w-4/12 hidden md:flex justify-between h-18'>
				{/* Language dropdown */}
				<div className='relative'>
					<button
						className='text-[#ccc] focus:outline-none p-3'
						onClick={() => setShowMenu(!showMenu)}
					>
						{<TfiWorld />}
					</button>
					<ul
						className={`${
							showMenu ? 'block' : 'hidden'
						} absolute -left-14 mt-1.5 py-2 w-40 bg-[#2a2a2a] text-[#ccc] text-center shadow-lg`}
					>
						{languages.map((language) => (
							<li key={language.code}>
								<button
									onClick={() => {
										handleLanguageChange(language.code);
										setShowMenu(false);
									}}
									className={`w-full py-2 ${
										selectedLanguage === language.code ? 'border-blue-500' : ''
									}`}
								>
									{language.label}
								</button>
							</li>
						))}
					</ul>
				</div>
				<button className='text-[#ccc] mx-3 focus:outline-none flex p-3'>
					<AiOutlineUserAdd className='m-1' />
					SIGN IN
				</button>
				<button className='bg-[#007aff] px-9 py-2 focus:outline-none text-[#ccc]'>
					DOWNLOAD
				</button>
			</div>
		</header>
	);
}
export default TopNav;
